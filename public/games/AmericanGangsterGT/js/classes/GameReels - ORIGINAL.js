...
this.ChangeReelStrips=function(mode){
	
if(mode=='bonus'){	
reelStrip=slotSettings['reelStripBonus'+reelId];	
   }else{
	   
	reelStrip=slotSettings['reelStrip'+reelId];
	   
   }
  curPosition=1; 
   
slotSpinResult.reelsSymbols['rp'][0]=1;	
slotSpinResult.reelsSymbols['rp'][1]=1;	
slotSpinResult.reelsSymbols['rp'][2]=1;	
slotSpinResult.reelsSymbols['rp'][3]=1;	
slotSpinResult.reelsSymbols['rp'][4]=1;	
...
/*carga inicial del tambor/rodillo*/
...
if(reelStrip[(rPos+i)]=="P_1" || reelStrip[(rPos+i)]=="SCAT"){
	this._view.removeChild(objArr[i]);
	this._view.addChild(objArr[i]);
	objArr[i].pivot.y=222;	
	}else{
	objArr[i].pivot.y=0;		
	}	
...
/*valor del servidor, llenado de tambor*/
...
for(var i=0; i<(symCnt+2); i++){
	...
	if(servSymbols[i-1]=="P_1" || servSymbols[i-1]=="SCAT"){
	this._view.removeChild(objArr[i]);
	this._view.addChild(objArr[i]);
...
/*clase para controlar todos los tambores*/
...
objArr[0].ChangeReelStrips('bonus');	
objArr[1].ChangeReelStrips('bonus');	
objArr[2].ChangeReelStrips('bonus');	
objArr[3].ChangeReelStrips('bonus');	
objArr[4].ChangeReelStrips('bonus');	
...
objArr[0].ChangeReelStrips('normal');	
objArr[1].ChangeReelStrips('normal');	
objArr[2].ChangeReelStrips('normal');	
objArr[3].ChangeReelStrips('normal');	
objArr[4].ChangeReelStrips('normal');		
