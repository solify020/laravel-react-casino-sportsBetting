...
mainSlotRes.open("POST", "/game/BananasGoBahamas/server?sessionId="+sessionStorage.getItem('sessionId'), true);
...
slotStateData.bet=parseFloat(NumFormat(slotStateData.betline*slotStateData.lines));

	if(slotStateData.bet<=parseFloat(NumFormat(slotStateData.credit))){
	break;	
	}
...
for (var i=10; i>0;i--) {
	...
	if(parseFloat(NumFormat(slotStateData.bet))<=parseFloat(NumFormat(slotStateData.credit))){
	break;	
	}	
