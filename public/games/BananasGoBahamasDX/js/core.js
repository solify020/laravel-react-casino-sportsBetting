...
Preloader.outerCircle=PIXI.Sprite.fromImage('/games/BananasGoBahamasDX/source/LOADER/loading.png');
...
Preloader.innerCircleArr[i]=PIXI.Sprite.fromImage('/games/BananasGoBahamasDX/source/LOADER/progress'+i+'.png');
...
window.parent.postMessage({
    ...
    'message': 'close'
	 ...
	 document.location.href=outQPar['api_exit'];
}
...
switch(ev.detail.bname){
	...
	slotStateData.lines+=2;
  	if(slotStateData.lines>9){
	...
	if(parseFloat(NumFormat(slotStateData.credit))>=parseFloat(NumFormat(slotStateData.bet))){
slotStateData.bet=parseFloat(NumFormat(slotStateData.bet));
slotStateData.credit=parseFloat(NumFormat(slotStateData.credit));
...
addEventListener("reelStop",function(ev){
	...
	if(ev.detail.reel==slotSettings.slotReelsConfig.length){
		...
		if(autoMode){
		autoMode=false;
		if(gameUI.GetObject()['uiButtonAuto'].ChangeLabel!=undefined){
